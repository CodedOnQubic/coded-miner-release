...
$CurrentVersion = [string]$script:CodedReleaseSelection.Version
$env:CODED_RELEASE_COMMIT = $CurrentCommit
$env:CODED_RELEASE_VERSION = $CurrentVersion
...
if ($selected -notin @("scalar","avx2","avx512","cuda")) { $selected = "scalar" }

# M1091V233_WINDOWS_CUDA_SINGLE_GPU_BASELINE
# The first productive CUDA beta owns one GPU scoring worker. FastDev tunes GPU
# batch/kernel parameters later; CPU thread fan-out must not create duplicate
# CUDA contexts or duplicate nonce work.
if ($selected -eq "cuda") {
  $Threads = 1
  if (-not $env:CODED_CUDA_DEVICE) { $env:CODED_CUDA_DEVICE = "0" }
}

$exe = Join-Path $dir ("coded-miner-{0}.exe" -f $selected)
if ($selected -eq "cuda" -and !(Test-Path $exe)) {
  throw "CUDA backend requested, but coded-miner-cuda.exe is not included in this Windows release."
}
if (!(Test-Path $exe)) {
...
# Every new CUDA beta commit must cross the exact packaged Full112 authority on
# the real GPU before productive pool mining starts. The pass stamp is bound to
# release commit + PTX + golden authority hashes, so channel autoupdate naturally
# re-runs the gate for a new beta without another user command.
if ($selected -eq "cuda") {
  $cudaPtx = Join-Path $dir "coded-bpp9000-cuda.ptx"
  $cudaGolden = Join-Path $dir "coded-bpp9000-cuda-golden.exe"
  $cudaGoldenMeta = Join-Path $dir "full112-v1.meta"
  $cudaTask = Join-Path $dir "task_bpp9000.bin"
  foreach ($required in @($cudaPtx,$cudaGolden,$cudaGoldenMeta,$cudaTask)) {
    if (!(Test-Path $required)) { throw "CUDA beta golden asset missing: $required" }
  }

  $env:CODED_BPP9000_CUDA_PTX_PATH = $cudaPtx
  $env:CODED_DIFFERENTIAL_GOLDEN_FILE = $cudaGoldenMeta
  ...
  $cudaGateKey = ([string]$cudaGateKey) -replace '[^A-Za-z0-9_.-]','_'
  $cudaStamp = Join-Path $cudaGateRoot ($cudaGateKey + ".json")
  $cudaStampValid = $false

  if (Test-Path $cudaStamp) {
    try {
  ...
  if (-not $cudaStampValid) {
    Write-Host "Running CUDA Full112 golden gate before beta mining..."
    $goldenOutput = @(& $cudaGolden --task $cudaTask --ptx $cudaPtx --golden $cudaGoldenMeta --device 0 2>&1)
    $goldenRc = $LASTEXITCODE
    $goldenOutput | ForEach-Object { Write-Host ([string]$_) }
    $goldenText = ($goldenOutput | ForEach-Object { [string]$_ }) -join "`n"
    if ($goldenRc -ne 0 -or $goldenText -notmatch "FINAL=PASS_CUDA_BETA_GOLDEN") {
      throw "CUDA beta Full112 golden gate failed rc=$goldenRc"
    }
    if ($goldenText -notmatch "architecture=blackwell") {
      throw "CUDA beta first-wave requires Blackwell hardware"
    }
    @{
      schema = "coded.windows.cuda-beta-golden-cache.v1"
    ...
    } | ConvertTo-Json -Compress | Set-Content -Path $cudaStamp -Encoding ASCII
  }

  Write-Host "CUDA beta golden gate: PASS | architecture=blackwell | backend=cuda"
}
